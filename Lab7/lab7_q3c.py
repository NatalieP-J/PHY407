...
psig = solve(eigenenergies[0])[1][0]
psi1 = solve(eigenenergies[1])[1][0]
psi2 = solve(eigenenergies[2])[1][0]

# Find the normalization constant for each constant
psigsq = 2*simpsons(abs(psig[:len(psig)/2])**2)
psi1sq = 2*simpsons(abs(psi1[:len(psi1)/2])**2)
psi2sq = 2*simpsons(abs(psi2[:len(psi2)/2])**2)
...
plt.xlabel('$x$ [$m$]')
plt.ylabel('Normalized $\psi (x)$ [$m^{-1}$]')
plt.xlim(-5*a,5*a)
