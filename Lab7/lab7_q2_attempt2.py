...
def Richardson(Rnm,Rn_1m,n,m):
	...
	return Rnm + err, np.sqrt(err[0]**2 + err[1]**2)

acc = 20*1e-10

def BSstep(r,fn,time,H,nmax):
	Rexts = []
	n = 1
	...
	while n < nmax:
		...
		Rs.append(Rn1)
		for j in range(1,n):
			#print j,n-1,Rexts,Rs
			m = j+1
			Rnm = Rs[j-1]
			Rn_1m = Rexts[(n-1)-1][j-1]
	...
	elif err > acc or np.isnan(err) == True:
		print 'Subdividing'
		r1 = BSstep(r,fn,time,H/2.,nmax)
		r2 = BSstep(r1,fn,time+H/2.,H/2.,nmax)
	return rval,err

print BSstep(np.array([0.,0.]),f,0.0,20.0,8)
